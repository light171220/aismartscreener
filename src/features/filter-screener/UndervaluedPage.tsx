...
import { useUndervaluedStocks, useRefreshFilteredStocks, type FilteredStock } from '@/hooks/useFilteredStocks';
import {
  TrendingDown,
  Target,
  RefreshCw,
      ...
      <div className="flex items-center gap-4 flex-1">
        <div className="min-w-[80px]">
          <div className="flex items-center gap-2">
            ...
            <GlassBadge variant="warning" size="sm">
              -{stock.belowLowTargetPct?.toFixed(0)}%
            </GlassBadge>
          </div>
        ...
        <div className="flex items-center gap-6 flex-1">
          ...
          <div className="text-center min-w-[100px]">
            <p className="font-mono text-amber-400">{formatCurrency(stock.lowTargetPrice || 0)}</p>
            <p className="text-xs text-slate-500">Low Target</p>
          </div>
...
export function UndervaluedPage() {
  const { data: stocks, isLoading, error, refetch } = useUndervaluedStocks();
  const { refresh } = useRefreshFilteredStocks();

  if (isLoading) {
    return <PageLoader message="Loading undervalued stocks..." />;
  }
  ...
  const avgDiscount = stocks && stocks.length > 0
    ? stocks.reduce((sum, s) => sum + (s.belowLowTargetPct || 0), 0) / stocks.length
    : 0;
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400">
            Stocks trading below analyst low price targets
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <GlassCard className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-amber-400">-{avgDiscount.toFixed(0)}%</p>
              <p className="text-xs text-slate-400">Avg Below Target</p>
            </div>
          </div>
        </GlassCard>
        <GlassCard className="p-4">
            ...
            <div>
              <p className="text-2xl font-bold text-white">
                {stocks && stocks.length > 0 
                  ? formatCurrency(stocks.reduce((sum, s) => sum + (s.lastPrice || 0), 0) / stocks.length)
                  : '$0'}
              </p>
      ...
      <GlassCard>
        <GlassCardHeader>
          <div className="flex items-center justify-between">
            <div>
              <GlassCardTitle>Undervalued Opportunities</GlassCardTitle>
              <GlassCardDescription>
                Sorted by discount to low target • Updated at 9:00 AM ET
              </GlassCardDescription>
            </div>
        ...
        <GlassCardContent>
          {!stocks || stocks.length === 0 ? (
            <div className="text-center py-12 text-slate-400">
              ...
              <p>No undervalued stocks found today</p>
...
export default UndervaluedPage;
